...
@ActiveProfiles("test")
class IngredientSearchLayerBenchmarkTest {
    ...
    fun `4단계 레이어별 통합 벤치마크`() {
        ...
        keywords.forEach { (keyword, group) ->
            ...
            try {
                ...
                val elapsed = System.currentTimeMillis() - start
                results.add(ResultEntry(keyword, group, "가드레일 차단", elapsed))
            } catch (e: Exception) {
                val elapsed = System.currentTimeMillis() - start
    ...
    private fun printReport(results: List<ResultEntry>) {
        ...
        println()
        println("========================================")
        println("  4단계 레이어별 통합 벤치마크 리포트")
        println("========================================")
        println()

        // --- 레이어별 집계 ---
        println("--- 레이어별 집계 ---")
        println("%-24s | %4s | %s".format("레이어", "건수", "비율"))
        println("-".repeat(50))
        layerOrder.forEach { layer ->
            val count = layerCounts.getOrDefault(layer, 0)
            val ratio = if (total > 0) count * 100.0 / total else 0.0
            println("%-24s | %4d | %5.1f%%".format(layerLabels[layer] ?: layer, count, ratio))
        }
        errorLayers.forEach { layer ->
            val count = layerCounts[layer]!!
            val ratio = count * 100.0 / total
            println("%-24s | %4d | %5.1f%%".format(layer, count, ratio))
        }
        println("-".repeat(50))
        println("%-24s | %4d | %5.1f%%".format("합계", total, 100.0))
        println()
        ...
        println("--- 그룹별 레이어 분포 ---")
        val headerLayers = allLayers.map { it.take(6) }
        println("%-16s | %s".format("그룹", headerLayers.joinToString(" | ") { "%4s".format(it) }))
        println("-".repeat(16 + 3 + allLayers.size * 7))
        groups.forEach { group ->
            val groupResults = results.filter { it.group == group }
            val counts = allLayers.map { layer -> groupResults.count { it.layer == layer } }
            println("%-16s | %s".format(group, counts.joinToString(" | ") { "%4d".format(it) }))
        }
        println()

        // --- 상세 결과 ---
        println("--- 상세 결과 ---")
        println("  %-12s | %-16s | %-18s | %s".format("키워드", "그룹", "레이어", "응답시간"))
        println("  " + "-".repeat(70))
        results.forEach { r ->
            println("  %-12s | %-16s | %-18s | %dms".format(r.keyword, r.group, r.layer, r.elapsedMs))
        }
        println()
    }
}
