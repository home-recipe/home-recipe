...
class VideoControllerTest {
    ...
    fun 영상_업로드_테스트_성공() {
        ...
        mockMvc.perform(
            MockMvcRequestBuilders.multipart("/api/admin/video").file(file)
        )
    ...
    fun 영상_업로드_테스트_파일_비었을_때_실패() {
        ...
        mockMvc.perform(multipart("/api/admin/video").file(emptyFile).with(csrf()))
    ...
    fun 랜덤_영상_목록_조회_테스트() {
        ...
        mockMvc.perform(get("/api/admin").with(csrf()))
            .andExpect(status().isOk)
